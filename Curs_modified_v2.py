...
def run_and_show_schedules():
    try:
        ...
        if num_buses <= 0 or num_drivers_a <= 0 or num_drivers_b <= 0:
        ...
        # Обновление текста метрик
        metrics_label.config(
            text=(
                f"Прямой алгоритм: Маршрутов = {straight_metrics[0]}, "
                f"Маршрутов в пик = {straight_metrics[1]}, Водителей = {straight_metrics[2]}\n"
                f"Генетический алгоритм: Маршрутов = {genetic_metrics[0]}, "
                f"Маршрутов в пик = {genetic_metrics[1]}, Водителей = {genetic_metrics[2]}"
            )
        )
        ...
        metrics_label.config(text=f"Ошибка: {e}")
    except Exception as e:
        metrics_label.config(text=f"Непредвиденная ошибка: {e}")
...
def save_schedule_to_file():
    try:
        ...
        if not output_file_name:
            metrics_label.config(text="Сохранение отменено.")
            return
        ...
        metrics_label.config(text=f"Расписание успешно сохранено в файл: {output_file_name}")

    except ValueError as e:
        metrics_label.config(text=f"Ошибка ввода данных: {e}")
    except Exception as e:
        metrics_label.config(text=f"Ошибка сохранения: {e}")
...
frame_inputs = ttk.Frame(notebook, padding=10)
notebook.add(frame_inputs, text="Параметры ввода")

# --- Вкладка: результаты ---
frame_results = ttk.Frame(notebook, padding=10)
notebook.add(frame_results, text="Результаты")

# --- Поля ввода (вкладка "Параметры ввода") ---
input_frame = ttk.LabelFrame(frame_inputs, text="Введите данные", padding=10)
...
date_entry = DateEntry(input_frame, width=12)
date_entry.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)

# --- Кнопка запуска алгоритмов ---
run_button = ttk.Button(frame_inputs, text="Сгенерировать расписание", command=lambda: print("Генерация..."))
run_button.pack(pady=10)

# --- Таблица для отображения расписания (вкладка "Результаты") ---
table_frame = ttk.LabelFrame(frame_results, text="Сводка расписания", padding=10)
table_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

schedule_table = ttk.Treeview(
    table_frame, columns=("Algorithm", "BusDriver ID", "BusSchedule", "Work Time", "Break Time"), show="headings"
)
...
metrics_frame = ttk.LabelFrame(frame_results, text="Метрики", padding=10)
metrics_frame.pack(fill=tk.X, padx=10, pady=10)

metrics_label = ttk.Label(metrics_frame, text="Тут будут отображены метрики после расчётов.")
metrics_label.pack(pady=5)

# --- Кнопка сохранения расписания ---
save_button = ttk.Button(frame_results, text="Сохранить расписание", command=lambda: print("Сохранение..."))
